...
> **One-time exception (v1.7+): a manual permission re-approval.** Starting with v1.7, the extension requests two new permissions (`scripting` and host access to eurognosi-fni.com) it never asked for before, to fix a "needs a refresh after a cold boot" issue (see [How it works](#how-it-works-technical-notes) → `background.js`). Browsers disable an already-installed extension when an update expands its permissions, until someone reviews and re-accepts them — the update itself still installs automatically, but the extension can sit disabled until that one click happens. If it appears disabled/greyed out after this specific update:
> - **Chrome/Edge:** go to `chrome://extensions` (or `edge://extensions`), find the extension, and click **"Details"** → allow the permissions it's asking to add.
> - **Firefox:** go to `about:addons` → **Extensions**, find it, and follow the prompt to allow its new permissions (Firefox may also show a toolbar notification for this directly).
>
> This is a one-time step for this one release; ordinary future updates go back to being fully silent.
...
> **Why Firefox gets its own manifest under the hood.** Chrome's Web Store validator and Firefox's linter want opposite things from the `background` key once a `service_worker` is declared (needed for the v1.7+ cold-boot fix, see [How it works](#how-it-works-technical-notes) → `background.js`): Chrome rejects the manifest outright if `background.scripts` is present under `manifest_version: 3`, while Firefox's linter errors if `background.service_worker` is present *without* a `background.scripts` fallback. `manifest.json` in the repo root is the Chrome/Edge-oriented one (`service_worker` only); `scripts/write-firefox-manifest.js` generates a Firefox-only variant (`scripts` instead of `service_worker`, everything else identical) that both `build-xpi.sh` and the release workflow's Firefox job stage into a temporary build before running `web-ext sign`/zipping. You never edit this variant by hand — it's always derived from `manifest.json`.
...
- `manifest.json` — restricts the extension to group pages on eurognosi-fni.com (including language-prefixed paths like `/en/group/...`), and loads the two script files below into the page. The `browser_specific_settings.gecko` block supplies the add-on ID Firefox requires and is ignored by Chromium browsers.
- `background.js` — a Manifest V3 background worker whose only job is to catch up tabs the normal content-script injection missed. On a cold boot (especially a busy machine reopening several tabs at once, or one you navigate to yourself in those first moments), the browser window can be usable before its extension subsystem has finished starting up — a page can start loading before the content script is registered, and the browser silently skips injecting into that tab. Previously this needed a manual refresh to fix. On `runtime.onStartup`/`onInstalled`, this worker queries already-open group-page tabs and injects `docx-lib.js` + `content.js` into any that don't have them yet (`content.js` sets `window.__egHomeworkSheetsLoaded` so this check, and the normal injection racing with it, can't double-run). This is what the `scripting` permission and `host_permissions` are for — the extension otherwise requests nothing beyond the pages it's scoped to. Chrome/Edge run it as a true `service_worker`; Firefox runs the exact same file as a `scripts`-declared background page instead, since the two stores require opposite things from that manifest key (see the note in [Releasing a new Firefox version](#releasing-a-new-firefox-version-auto-update-pipeline)) — either way, the same catch-up behavior applies on both.
