...
- `background.js` — a Manifest V3 background service worker whose only job is to catch up tabs the normal content-script injection missed. On a cold boot (especially a busy machine reopening several tabs at once, or one you navigate to yourself in those first moments), the browser window can be usable before its extension subsystem has finished starting up — a page can start loading before the content script is registered, and Chrome silently skips injecting into that tab. Previously this needed a manual refresh to fix. On `runtime.onStartup`/`onInstalled`, this worker queries already-open group-page tabs and injects `docx-lib.js` + `content.js` into any that don't have them yet (`content.js` sets `window.__egHomeworkSheetsLoaded` so this check, and the normal injection racing with it, can't double-run). This is what the `scripting` permission and `host_permissions` are for — the extension otherwise requests nothing beyond the pages it's scoped to. Declared via `background.service_worker`, the only key Chrome's Manifest V3 validator accepts (it rejects the older MV2 `background.scripts` key outright when `manifest_version` is 3) — older Firefox versions that don't yet support `service_worker` backgrounds will simply skip loading it, so the cold-boot fix is Chrome/Edge-only until Firefox's support for it is broad enough to rely on; Firefox users just keep the existing "refresh once after a cold boot" behavior.
